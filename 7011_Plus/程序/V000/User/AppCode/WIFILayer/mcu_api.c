...
{
	extern unsigned char is_read_time;
...
返回参数 : WIFI_WORK_SATE_E:
                            SMART_CONFIG_STATE: smartconfig 配置状态 间隔闪烁250ms
                                      AP_STATE: AP          配置状态 间隔闪烁1500ms
                            WIFI_NOT_CONNECTED: WIFI        配置成功但未连上路由器 长暗
                                WIFI_CONNECTED: WIFI        配置成功且连上路由器 长亮
                               WIFI_CONN_CLOUD: WIFI        已经连接上云服务器 长亮
                                WIFI_LOW_POWER: WIFI        处于低功耗模式 长暗
使用说明 : 无
*****************************************************************************/
...
//  #error "请直接在main函数的while(1){}中添加wifi_uart_service(),调用该函数不要加任何条件判断,完成后删除该行" 
  ...
  if(rx_in < PROTOCOL_HEAD)	return;						// 如果收到的数据个数小于7个
