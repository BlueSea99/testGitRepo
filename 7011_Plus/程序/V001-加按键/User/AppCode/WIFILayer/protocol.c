...
//  #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_bool_update(DPID_LED_SWITCH,当前开关); //BOOL型数据上报;
    mcu_dp_value_update(DPID_BRIGHT_VALUE,当前亮度); //VALUE型数据上报;

    */
	extern u8 Cloud;
	extern u8 Digital;
	extern bit Logic_Switchs_Value;
    
	Data_s	Switch;

	Switch.led_switch 		= Dimmer.FaceValue;
	Switch.bright_value 	= SwitchF ? 1 : 0;

	mcu_dp_bool_update(DPID_LED_SWITCH,Switch.led_switch); 		//BOOL型数据上报;
	mcu_dp_value_update(DPID_BRIGHT_VALUE,Switch.bright_value); 	//VALUE型数据上报;	
}
...
static unsigned char dp_download_bright_value_handle(const unsigned char value[], unsigned short length)
{
  ...
  Dimmer.FaceValue = BrightApp2Mcu(bright_value);
    
  if(Dimmer.FaceValue < LightValTab[Dimmer.MinLightVal][0])
  {
    Dimmer.FaceValue = LightValTab[Dimmer.MinLightVal][0];
  }
  
  if(Dimmer.FaceValue >= 150) 
  {
    Dimmer.FaceValue = LightValTab[Dimmer.MinLightVal][9];
  }      
  ...
  ***********************************/
  ...
  switch(dpid)
  {
    case DPID_LED_SWITCH:
      //开关处理函数
      ret = dp_download_led_switch_handle(value,length);
      break;
