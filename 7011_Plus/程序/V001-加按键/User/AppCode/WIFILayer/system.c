...
void data_handle(unsigned short offset)
{
    ...
    switch(cmd_type)
    {
        case HEAT_BEAT_CMD:             // 0 心跳包 (模组发送，MCU上报，成功则模组间隔15s发送一次)
            heat_beat_check();
        break;
    
        case PRODUCT_INFO_CMD:          // 1 产品信息 (模组发送，MCU上报，PID号/mcu版本号/配网方式)
            product_info_update();
            break;
    
        case WORK_MODE_CMD:             // 2 查询MCU设定的模块工作模式 (？模组发送，MCU上报)
            get_mcu_wifi_mode();
            break;
    
#ifndef WIFI_CONTROL_SELF_MODE
        case WIFI_STATE_CMD:            // 3 报告wifi工作状态	
  ...
  case WIFI_RESET_CMD:                  // 4 重置wifi(wifi返回成功)
    reset_wifi_flag = RESET_WIFI_SUCCESS;
    break;
    
  case WIFI_MODE_CMD:                   // 5 选择smartconfig/AP模式(wifi返回成功)	
    set_wifimode_flag = SET_WIFICONFIG_SUCCESS;
    break;
#endif
    
  case DATA_QUERT_CMD:                  // 6 广播 (模组发送，MCU上报，自定义的数据如亮度，开关量)
...
#ifdef WIFI_TEST_ENABLE
  case WIFI_TEST_CMD:                                   // 0x0e wifi功能测试（扫描指定路由）
